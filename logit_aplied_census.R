...
clas = glm(formula=income ~. , family = binomial, data=base_train )
summary(clas)

prev = predict(clas, type = 'response', newdata = base_test)

income = ifelse(prev>0.5, 1, 0)



#-------------- Confusion Matrix

conf_matrix = table(base_test[ ,15], income)
